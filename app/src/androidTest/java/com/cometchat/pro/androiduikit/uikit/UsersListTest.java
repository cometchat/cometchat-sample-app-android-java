...
public class UsersListTest {
    ...
    public void setup() {
        ...
        Fragment userList = new CometChatUserList();
        CometChatUI.getCometChatUIActivity()
                .getSupportFragmentManager().beginTransaction()
                .replace(CometChatUI.getBinding().frame.getId(),userList).commit();
        try {
            Thread.sleep(3000);
            view = userList.getView();
        } catch (Exception e) {
    ...
    @Test
    public void A_checkUserListNotEmpty() {
        RecyclerView userList = (RecyclerView)view.findViewById(R.id.rv_user_list);
        ...
        if (adapter.getItemCount()>1) {
            ...
            Assert.fail("UserList is Empty");
        }
    }

    @Test
    public void A_checkUserListLastItem() {
        RecyclerView userList = (RecyclerView)view.findViewById(R.id.rv_user_list);
        RecyclerView.Adapter adapter = userList.getAdapter();
        if (adapter.getItemCount()>1) {
            Espresso.onView(Matchers.allOf(
